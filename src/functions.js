...
export const addToFavorites = (favoriteHeroId, event) => {
  ...
  let favoriteHeroAddFavoritedClass = document.querySelector(`[data-id="${favoriteHeroId}"]`).querySelector('.heroName');
  ...
  if (favoriteHeroes.includes(favoriteHeroId)) {
    ...
    favoriteHeroAddFavoritedClass.classList.remove('favorited');
  } else {
    if (favoriteHeroes.length < 5) {
      favoriteHeroes.push(favoriteHeroId);
      event.target.parentNode.classList.add('favorited');

      favoriteHeroAddFavoritedClass.classList.add('favorited');
    } else {
